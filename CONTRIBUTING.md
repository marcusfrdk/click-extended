...
- Avoid creating too many files if possible (try to include tests within the same category in the same file).
...
All code must pass strict type checking. Add type hints to all functions and classes. The `make type` command checks both the `click_extended` source code and the `tests` directory.
